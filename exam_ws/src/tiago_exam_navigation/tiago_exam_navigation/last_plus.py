...
class BlueBlockNavigator(Node):
    def __init__(self):
        ...
        self.head_pub = self.create_publisher(Float64, '/head_controller/command', 10)
    ...
    def image_callback(self, msg):
        ...
        if contours:
            ...
            if abs(block_center_x - self.image_width // 2) > 30:
                self.align_block_with_head(block_center_x)
            else:
    ...
    def align_block_with_head(self, block_center_x):
        """Regola la testa per centrare il blocco nell'immagine."""
        error = block_center_x - self.image_width // 2  # Differenza tra centro blocco e centro immagine
        head_command = Float64()

        # Controllo dell'errore per allineare la testa
        head_command.data = -0.2 if error > 0 else 0.2

        # Pubblica il comando per ruotare la testa
        self.head_pub.publish(head_command)
        self.get_logger().info(f"🔄 Allineamento testa in corso: errore={error}")
